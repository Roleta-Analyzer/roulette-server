...
// Criar servidor HTTP para responder ao health check do Railway
// (sem isso, Railway reinicia o serviço e derruba conexões com código 1006)
const httpServer = http.createServer((req, res) => {
    if (req.method === 'GET' && (req.url === '/' || req.url === '/health')) {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({
            status: 'ok',
            service: 'roulette-websocket-server',
            roulettes: availableRoulettes ? availableRoulettes.size : 0,
...
// Criar WebSocket server anexado ao servidor HTTP (sem porta própria)
const wss = new WebSocket.Server({ server: httpServer });

// Iniciar servidor HTTP (e WebSocket via upgrade)
...
if (fs.existsSync(configPath)) {
...
} else if (process.env.WEBSOCKET_URL) {
    // Fallback: carregar configuração via variáveis de ambiente (Railway/produção)
    console.log('📋 api-config.js não encontrado — usando variáveis de ambiente');
    ...
    API_CONFIG = {
        ...
        parseRoulettesResponse: (r) => (r.game && r.game_type === 'roleta' && Array.isArray(r.results)) ? [r.game] : [],
        parseHistoryResponse: (r) => {
            if (r.game && r.game_type === 'roleta' && Array.isArray(r.results)) {
                return r.results.map(n => { if (n === '00') return 0; const num = parseInt(n); return isNaN(num) || num < 0 || num > 36 ? 0 : num; });
            }
            return [];
        },
        parseNewNumberMessage: (msg) => {
            if (msg.game && msg.game_type === 'roleta' && Array.isArray(msg.results) && msg.results.length > 0) {
                const num = msg.results[0] === '00' ? 0 : parseInt(msg.results[0]);
                return { number: isNaN(num) || num < 0 || num > 36 ? 0 : num, roulette: msg.game };
            }
            return null;
        },
        timeout: 10000,
    ...
    console.error('❌ ERRO: api-config.js não encontrado e variável WEBSOCKET_URL não definida!');
    console.error('💡 Opção 1: Crie api-config.js com a URL do WebSocket');
    console.error('💡 Opção 2: Defina a variável de ambiente WEBSOCKET_URL no Railway');
    console.error('   Exemplo: WEBSOCKET_URL=wss://websocket-server-production-6597.up.railway.app?token=SEU_TOKEN');
    process.exit(1);
}
...
async function hydrateFromStore(rouletteId) {
    ...
    if (existingHistory && existingHistory.length > 0) {
        // Cache já populado — silencioso para não gerar rate limit no Railway
        return;
    }
...
function startApiHealthCheck() {
    ...
    apiHealthCheckInterval = setInterval(() => {
        ...
        const sinceMsg = timeSinceLastMessage ? Math.floor(timeSinceLastMessage / 1000) + 's' : 'NUNCA';
        
        // Log compacto (1 linha) para reduzir rate limit no Railway
        console.log(`🏥 HealthCheck | status:${apiConnectionStatus} ws:${readyStateText} msgs:${apiMessageCount} últimaMsg:${sinceMsg} roletas:${availableRoulettes.size} clientes:${wss.clients.size}`);
        ...
        if (apiConnectionStatus === 'connected' && timeSinceLastMessage && timeSinceLastMessage > 120000) {
            console.warn('⚠️ API TRAVADA: 2+ minutos sem mensagens - fechando WS para reconexão');
            if (apiWebSocket) {
                try {
                    apiWebSocket.close(); // o evento 'close' cuidará da reconexão
                } catch (e) {
...
function connectToAPIWebSocket() {
    // Guard: evitar race condition — não reconectar se já está conectando
    if (apiWebSocket && (apiWebSocket.readyState === 0 || apiWebSocket.readyState === 1)) {
        const stateText = apiWebSocket.readyState === 0 ? 'CONNECTING' : 'OPEN';
        console.log(`⚠️ connectToAPIWebSocket() ignorado — conexão já está ${stateText}`);
        return;
    }
    ...
    try {
        // Usar referência local para evitar race condition quando apiWebSocket é sobrescrito
        const currentWs = new WebSocket(wsUrl);
        apiWebSocket = currentWs;

        currentWs.on('open', () => {
            // Checar se esta ainda é a conexão ativa (evitar evento de conexão obsoleta)
            if (currentWs !== apiWebSocket) {
                console.warn('⚠️ Evento open de conexão obsoleta — ignorando e fechando');
                currentWs.close();
                return;
            }
            ...
            try {
                currentWs.send(JSON.stringify({ type: 'get_roulettes', action: 'list_tables' }));
        ...
        currentWs.on('message', async raw => {
            // Ignorar mensagens de conexões obsoletas
            if (currentWs !== apiWebSocket) return;

            lastApiMessageTime = Date.now();
            apiMessageCount++;
            
            // Log apenas primeiras 5 mensagens para confirmar funcionamento
            if (apiMessageCount <= 5) {
                console.log(`\n🔥 API MENSAGEM #${apiMessageCount}:`);
                console.log(`   📏 Tamanho: ${raw.length} bytes`);
                console.log(`   📄 Preview: ${raw.toString().substring(0, 200)}`);
            }
            
            try {
                const message = JSON.parse(raw.toString());

                // Log periódico de status (a cada 500 mensagens para reduzir logs)
                if (apiMessageCount % 500 === 0) {
                ...
                if (API_CONFIG.verbose) {
                    console.log('📨 Mensagem da API (game):', message.game || 'N/A');
                }
